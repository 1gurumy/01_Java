...
public class FileService {
	 ...
	 * 이스케이프 문자 사용하려면 \(한번), 역슬래시 입력하고싶으면 \\(두번) 입력해야함.
	 ...
	 * 				 목표까지의 경로를 모두 표기하는 방법 // C:\workspace\01_Java
	 *
	 * - 기준
	 * [Windows] :   C:/  , D:/
	 * [Linux, Mac] : /Users ,   /
	...
	public void method1() {
		...
		// 리눅스, mac에서는 Users/		// 윈도우에서는 C:/ 혹은 D:/ 등
		
		// 	C:/io_test/20250305 폴더를 지정
		File directory = new File("/io_test/20250305");
		
		// 존재하지 않는 폴더를 관리하는 File 객체를 생성
		
		if(!directory.exists()) {	// 폴더가 존재하지 않으면
			
			// 폴더를 모두 생성하기 	모두 생성하는 거니까 mkdir말고  mkdirs 메서드 사용해야함.
			directory.mkdirs();
			
			System.out.println(directory.getName());	// 가장 하위에 있는 폴더명 반환
			System.out.println(directory.getPath());	// 모든 경로 반환
		}
	...
	public void method2() {
		...
		// 문자열에서 "\"를 표기하는 방법은 "\\" 두 개 입력
		// -> 역슬래시 하나는 이스케이프 문자로 사용됨.
		// -> 따라서 역슬래시 자체를 문자로 사용하고 싶다면 2개를 붙여야 한다.
		File file = new File("\\io_test\\20250305\\파일생성.txt");
		
		if(!file.exists()) {	// \\io_test\\20250305\\파일생성.txt 위치에 파일이 존재하지 않으면
			
			
			// 파일 생성
			try {
				// File 클래스의 메서드는 대부분 IOException을 발생시킴.
				// -> IOException은 Checked Exception에 속하므로 예외처리가 필수임. 
				...
				e.printStackTrace();	
				// 예외 정보 + 예외가 발생한 위치까지의 메서드 추적해주는 메서드
			}
		}
	...
	public void method3() {	
		...
		File[] files = directory.listFiles();	// File[]   listFiles() : 디렉토리 내 파일 목록을 File[] 배열로 반환
		
		System.out.println(Arrays.toString(files));	//	Arrays.toString(배열명) :  배열이 가진 값 나열해주는 메서드
				
		// 향상된 for문
		for(File f : files) {
			
			// 파일명 얻어오기
			String fileName = f.getName();	// String   getName() : 파일 이름 반환
			
			// 마지막으로 수정한 날짜 얻어오기
			Long lastModified = f.lastModified();	//long   lastModified() : 파일 마지막 수정일 (1970.01.01 09:00 부터 현재까지 지난 시간을 ms 단위로 반환)
			...
			// -> 만약 구하려면 폴더 안의 모든 파일을 순회하여 각 파일의 크기를 합산해서 써야함.
