...
public class ArrayExample1 {
	 ...
	 *   구분은 index를 이용함 (index는 0부터 시작하는 정수, 배열의 각 칸을 의미)
	 * 
	 * */
	...
	public void ex1() {
		...
		// 변수 선언
		int num; 
		// 해석 : 메모리 구조 중 stack 영역에 int 자료형을 저장할 수 있는 공간 4byte를 할당하고
		// 그 공간에 num이라는 이름을 부여.
		
		// 변수 대입
		num = 10;
		// 해석 : 생성된 num이라는 변수 공간에 10이라는 정수값 대입
	...
	public void ex2() {
		...
		// 0번 index부터 각 요소에 1~5까지 초기화함(대입)
	...
	public void ex4() {
		...
		for(int i = 0; i < score.length; i++) {
		 ...
		 // ====================================
		 // -> score 배열의 각 요소가 모두 입력한 값으로 대입 완료된 시점
	...
	public void ex6() {
		...
		// 신호를 나타내기 위한 변수
		boolean flag = false; // 검사 전에는 일치하는 값이 없다고 가정
		...
		// flag 상태를 검새
		// flag가 flase라면 일치하는 값을 찾지 못했다라는 의미
		// 그런데 flag가 false라면 of 조건식이 false이므로 수행되지 않음
	...
	public void ex7() {
		...
		String[] arr= {"사과", "딸기", "바나나", "키위", "메론", "아보카도"};
		...
		for(int i = 0; i < arr.length; i++) {
			...
			if ( arr [i].equals(input) ) { // Stirng 비교 시 equals() 사용! 
				System.out.println(i + "번째 인덱스에 존재");
				flag = false;
			} 
		}
	...
	public void ex8() {
		...
		String str = "가나다"; // Stirng Constant Pool(문자열 상수풀)에 저장된다.
		String str2 = new String("가나다"); // Heap 새 객체로서 저장
		
		System.out.println(str); // 주소값? -> x
		System.out.println(str2);
		System.out.println(str.toString()); 
		
		// String 클래스의 toString() 메서드가 존재
		// toString() 메서드는 자기 자신을 그대로 반환하게 끔 만들어짐.
	...
	public void ex9 () {
		...
		// 2. 문자 하나를 더 입력받아 일치하는 문자가 char 배열에 몇 개 존재하는지 확인
